...
## Assignments & homework (Phase 10)

- **Class assignments** — class managers (org owners/admins and the class teacher) assign any lesson from the class's course to the whole class with a due date and optional note (`ClassAssignment` + `AssignmentSubmission` tables, migration `20260816125140_add_class_assignments`).
- **Student completion** — enrolled students mark an assignment done (and can undo it); non-members and members of other orgs are rejected. Class pages and the dashboard show due dates with overdue highlighting and per-class completion (`x/y done`).
- **Assignments hub** — `/app/assignments` shows a role-scoped list: org managers see every class's assignments, teachers their own classes, students their enrolled classes, with completion counts and a live done/not-done toggle.
- **Home dashboard** — the learner dashboard now shows an "Upcoming assignments" card (open, non-overdue assignments across classes) linking into the lesson viewer.
- **Loaders & actions** — `loadClassAssignments`, `loadAssignmentsForUser` (org/teaching/enrolled scopes) and `loadCourseLessonsForClass` in `src/lib/server/assignments.ts`; `createAssignment`, `deleteAssignment` and `setAssignmentDone` in `src/lib/server/actions/assignments.ts` with audit entries (`assignment.created` / `assignment.deleted`). Assignments derive their lesson title live, so deleting a lesson cascades its assignments.
- **Coverage** — integration tests in `tests/assignments.test.ts` (7 tests): class-manager create with audit, wrong-lesson/wrong-course and bad-date rejection, student create/delete forbidden, enrolled-only completion with undo, org/teaching/enrolled scoping, lesson options, and manager delete.

## Notifications & activity feed (Phase 11)

- **Activity feed** — every important write (course/class/assignment/student events, publish state changes, topic & lesson edits) appends an event to an org-wide feed (`ActivityEvent` table, migration `20260816132046_add_activity_feed`). `/app/activity` shows the newest 50 with icons, linked names and relative timestamps.
- **Privacy-aware visibility** — staff see every event; learners only see org-wide events, events about themselves, and assignment events for classes they are enrolled in (`filterForLearner` in `src/lib/server/activity.ts`) — never classmates' completions or roster changes.
- **Personal notifications** — targeted notifications with read state (`Notification` table): assignment created → every enrolled student, assignment completed → the class teacher, student enrolled → the student. The bell icon in the app shell shows the unread count (capped at `9+`) and `/app/notifications` lists items with unread highlighting and a "Mark all read" button.
- **Best-effort recording** — `recordActivity` wraps event + notification writes in one transaction and never fails the caller, mirroring `recordAudit`.
- **Bug fix** — the `enrollStudent` action previously used a staff-only member lookup and could never enroll an actual student; it now validates via a dedicated `resolveActiveStudent` (student-role members).
- **Coverage** — integration tests in `tests/activity.test.ts` (6 tests): assignment-created notifications to enrolled students only, teacher notified on completion, student notified on enrollment, learner feed visibility vs. non-enrolled learners vs. staff, mark-read scoping (own only + specific ids), and org isolation.

## Invitations & member management (Phase 12)

- **Invitations** — org owners/admins invite new members by email and role via the `inviteMember` action, which creates an `Invitation` record with a 7-day token-based link. The existing `/invite/[token]` page handles the accept flow: sign up → visit link → membership created, current org switched, `ACCEPTED` recorded.
- **Pending invitations panel** — the Account page's Members card now shows pending invitations below the member list, with a "Copy link" button (clipboard) and a "Revoke" button for managers. Revoked invitations are greyed out and cannot be accepted.
- **`listInvitations`** — returns all invitations (pending, revoked, accepted, expired) for an org, scoped by `member.view` permission.
- **Activity feed integration** — `inviteMember`, `acceptInvitation` and `removeMember` now append org-wide feed events (`member.invited`, `member.joined`, `member.removed`) via `recordActivity`, visible to staff and learners alike.
...
## Landing page rebrand (Phase 14)

- **Brand** — "DeutschMeister" rebranded to "Lernio" across the landing page, root layout metadata (title, description, OpenGraph, Twitter, keywords), and the learner sidebar logo.
- **Hero** — repositioned from individual German exam prep to multi-tenant SaaS: "The learning platform your team actually uses." with org-focused CTAs ("Create your school").
- **Features** — replaced individual learner features (study plan, mock exams, etc.) with platform features: multi-tenant organizations, course & class management, progress & analytics, notifications & activity feed, roles & permissions, AI-powered learning.
- **How it works** — replaced individual setup steps with org onboarding flow: create org → invite team → track and grow.
- **Footer** — updated branding and links (/account, /login).
- **Coverage** — presentational-only page (no server logic, no tests needed). Verified via tsc, build and smoke test.
